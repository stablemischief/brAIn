...
class ProcessingQuality(BaseModel):
    ...
    confidence_score: float = Field(ge=0.0, le=1.0, description="Confidence in extraction quality")
    text_length: int = Field(ge=0, description="Length of extracted text")
    estimated_accuracy: float = Field(ge=0.0, le=1.0, description="Estimated accuracy of extraction")
    extraction_method: str = Field(description="Method used for extraction")
    format_specific_metrics: Dict[str, Any] = Field(default_factory=dict, description="Format-specific quality metrics")
...
class TextChunk(BaseModel):
    ...
    quality_score: float = Field(ge=0.0, le=1.0, description="Quality assessment for this chunk")
...
class FileProcessingResult(BaseModel):
    ...
    content_language: Optional[str] = Field(default=None, description="Detected content language")
    ...
    detected_links: List[str] = Field(default_factory=list, description="Detected hyperlinks")
...
class ProcessingConfig(BaseModel):
    ...
    chunk_size: int = Field(default=400, ge=50, le=8000, description="Default chunk size in characters")
    chunk_overlap: int = Field(default=0, ge=0, le=200, description="Overlap between chunks")
    ...
    supported_formats: List[str] = Field(default_factory=lambda: [
        'pdf', 'docx', 'doc', 'xlsx', 'xls', 'pptx', 'txt', 'html', 'csv'
    ])
...
class EnhancedTextProcessor:
    ...
    def get_openai_client(self) -> OpenAI:
        ...
        if self.openai_client is None:
            ...
            if api_key:
                ...
                raise ValueError("EMBEDDING_API_KEY or OPENAI_API_KEY not found in environment variables")
    ...
    def estimate_processing_cost(self, text: str, model: str = "text-embedding-3-small") -> Decimal:
        ...
        rate = cost_per_1k_tokens.get(model, cost_per_1k_tokens["text-embedding-3-small"])
    ...
    def sanitize_text(self, text: str) -> str:
        ...
        if len(text) < original_length * 0.8:
            print(f"Warning: Significant text loss during sanitization ({original_length} -> {len(text)})")
    ...
    def assess_extraction_quality(self, original_size: int, extracted_text: str, 
                                extraction_method: str, warnings: List[str], 
                                errors: List[str]) -> ProcessingQuality:
        ...
        return ProcessingQuality(
            ...
            format_specific_metrics={
                ...
                'has_structured_content': any(marker in extracted_text.lower() 
                                            for marker in ['table', 'sheet', 'column', 'row'])
            }
        )
    
    def create_validated_chunks(self, text: str, chunk_size: Optional[int] = None, 
                              overlap: Optional[int] = None) -> List[TextChunk]:
    ...
    def extract_text_from_docx(self, file_content: bytes) -> Tuple[str, List[str], List[str]]:
                ...
                if 'word/document.xml' in zipf.namelist():
                    with zipf.open('word/document.xml') as xml_file:
                        ...
                        if len(extracted_text) < 50 and len(file_content) > 1000:
                            warnings.append("Very little text extracted from large DOCX file")
    ...
    def extract_text_from_pdf(self, file_content: bytes) -> Tuple[str, List[str], List[str]]:
        ...
        try:
            ...
            if len(extracted_text) < 100 and len(file_content) > 10000:
                warnings.append(f"Very little text extracted from PDF using {method_used}")
    ...
    def extract_text_from_file(self, file_content: bytes, mime_type: str, 
                             file_name: str) -> Tuple[str, str, List[str], List[str]]:
        """
        ...
        try:
            ...
            if mime_type == 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' or file_name.endswith('.docx'):
                text, method_warnings, method_errors = self.extract_text_from_docx(file_content)
                method = "docx_xml"
                warnings.extend(method_warnings)
                errors.extend(method_errors)
                
            elif mime_type == 'application/pdf' or file_name.endswith('.pdf'):
                text, method_warnings, method_errors = self.extract_text_from_pdf(file_content)
                method = "pdf_extraction"
                warnings.extend(method_warnings)
                errors.extend(method_errors)
                
            elif mime_type in ['text/plain', 'text/html', 'text/csv'] or file_name.endswith(('.txt', '.html', '.csv')):
                try:
    ...
    def process_file_with_validation(self, file_content: bytes, file_name: str, 
                                   file_id: Optional[str] = None, 
                                   mime_type: Optional[str] = None) -> FileProcessingResult:
        """
        ...
        if file_size > max_size:
            raise ValidationError(f"File size {file_size} exceeds maximum {max_size} bytes")
        
        # Extract text with quality tracking
        extracted_text, extraction_method, warnings, errors = self.extract_text_from_file(
