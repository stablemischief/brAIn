...
class CostSystem:
    ...
    def __init__(self, config: Optional[CostSystemConfig] = None):
        ...
        if self.config.enable_budget_enforcement:
            ...
            self.budget_enforcer = get_budget_enforcer(
                self.cost_calculator, alert_callbacks=[self._handle_budget_alert]
            )
            if budget_config:
        ...
        self.analytics = get_cost_analytics(
            self.cost_calculator, self.budget_enforcer, self.optimizer
        )
    ...
    async def process_operation(
        ...
        try:
            ...
            if enable_optimization and self.config.enable_auto_optimization:
                try:
                    optimization_result = self.optimizer.optimize_operation(
                        ...
                        or self.config.quality_threshold,
                    )
                    recommended_model = optimization_result.recommended_model
                    logger.info(
                        f"Optimized model selection: {recommended_model} for operation {operation_id}"
                    )
                except Exception as e:
                    logger.warning(
                        f"Optimization failed for operation {operation_id}: {e}"
                    )
            ...
            cost_category = category or self._infer_category_from_operation_type(
                operation_type
            )
            ...
            if self.budget_enforcer:
                allowed, violations, approval_requests = (
                    self.budget_enforcer.check_budget_compliance(
                ...
                budget_compliance = {
                    ...
                    "approval_requests": (
                        [asdict(req) for req in approval_requests]
                ...
                if not allowed:
                    logger.warning(
                        f"Operation {operation_id} blocked by budget enforcement"
                    )
            ...
            return {
                ...
                "optimization": {
                    ...
                    "recommendation": (
                        asdict(optimization_result) if optimization_result else None
                    ),
        ...
        try:
            ...
            if enable_optimization:
                try:
                    ...
                    logger.info(
                        f"Batch optimization completed: {optimization_result.savings_percentage:.1f}% savings"
                    )
                except Exception as e:
            ...
            for i, op in enumerate(operations):
                ...
                if optimization_result:
                    recommended_model = optimization_result.model_assignments.get(
                        f"op_{i}"
                    )
            ...
            successful_operations = [
                r for r in operation_results if r["status"] == "completed"
            ]
            failed_operations = [
                r for r in operation_results if r["status"] != "completed"
            ]
            ...
            return {
                ...
                "summary": {
                    ...
                    "average_cost_per_operation": (
                        float(total_cost / len(successful_operations))
    ...
    def get_real_time_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active_alerts = [
                asdict(alert)
                for alert in self.analytics.alerts.values()
            ...
            return {
                ...
                "system_health": {
                    ...
                    "cache_size": (
                        len(self._operation_cache) if self._operation_cache else 0
                    ),
    ...
    def get_cost_forecast(
        ...
        forecast = self.optimizer.forecast_costs(
            period, confidence_threshold=confidence_threshold
        )
    ...
    def cleanup_old_data(self, days_to_keep: Optional[int] = None):
        ...
        if self._operation_cache:
            cache_cutoff = datetime.now() - timedelta(
                minutes=self.config.cache_ttl_minutes
            )
            old_keys = [
                key
                for key, data in self._operation_cache.items()
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "statistics": {
                ...
                "total_cost": float(
                    sum(c.total_cost for c in self.cost_calculator.cost_history)
                ),
                "cache_size": (
                    len(self._operation_cache) if self._operation_cache else 0
                ),
                "active_alerts": len(
                    [a for a in self.analytics.alerts.values() if not a.resolved]
                ),
            },
        }
...
async def process_document_batch(
    documents: List[Dict[str, Any]], enable_optimization: bool = True, **kwargs
) -> Dict[str, Any]:
    """Process multiple documents as an optimized batch"""

    system = get_cost_system()
    return await system.process_batch(
        operations=documents, enable_optimization=enable_optimization, **kwargs
    )
