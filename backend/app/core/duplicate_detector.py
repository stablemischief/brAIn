...
class DeduplicationResult(BaseModel):
    ...
    duplicate_groups: List[List[str]] = Field(default_factory=list, description="Groups of duplicates")
    duplicates_removed: int = Field(ge=0, description="Number of duplicates removed")
    processing_time: float = Field(ge=0.0, description="Processing time in seconds")
    cost_savings: Decimal = Field(ge=0, description="Estimated cost savings from deduplication")

class DuplicateDetectionConfig(BaseModel):
    """Configuration for duplicate detection."""
    enable_hash_detection: bool = Field(default=True, description="Enable exact hash matching")
    enable_vector_similarity: bool = Field(default=True, description="Enable vector similarity")
    enable_fuzzy_matching: bool = Field(default=True, description="Enable fuzzy text matching")
    similarity_threshold: float = Field(default=0.95, ge=0.0, le=1.0, description="Similarity threshold")
    fuzzy_threshold: float = Field(default=0.90, ge=0.0, le=1.0, description="Fuzzy matching threshold")
    min_content_length: int = Field(default=50, ge=1, description="Minimum content length to check")
    batch_size: int = Field(default=100, ge=1, le=1000, description="Batch size for processing")
    enable_content_normalization: bool = Field(default=True, description="Enable content normalization")
...
class DuplicateDetectionEngine:
    ...
    def detect_exact_duplicates(self, content_items: List[Tuple[str, str]]) -> List[DuplicateMatch]:
        """
    ...
    def detect_similarity_duplicates(self, content_items: List[Tuple[str, str]], 
                                   embeddings: Optional[Dict[str, List[float]]] = None) -> List[DuplicateMatch]:
        """
        ...
        Returns:
            List of duplicate matches
        """
        if not self.config.enable_vector_similarity and not self.config.enable_fuzzy_matching:
            return []
        ...
        for i, (id1, content1) in enumerate(content_items):
            ...
            for j, (id2, content2) in enumerate(content_items[i + 1:], start=i + 1):
                ...
                if (self.config.enable_vector_similarity and 
                    embeddings and 
                    id1 in embeddings and 
                    id2 in embeddings):
                ...
                if self.config.enable_fuzzy_matching:
                    ...
                    if len(content1) < 2000 and len(content2) < 2000:
                        levenshtein_sim = self.calculate_levenshtein_ratio(content1, content2)
    ...
    def check_document_duplicate(self, content_hash: str, content: str) -> bool:
        ...
        try:
            ...
            if self.config.enable_hash_detection:
                hash_match = self.db_handler.supabase.table("documents").select(
                    "id"
            ...
            if (self.config.enable_vector_similarity and 
                len(content) >= self.config.min_content_length):
    ...
    def deduplicate_content_batch(self, content_items: List[Tuple[str, str]], 
                                embeddings: Optional[Dict[str, List[float]]] = None) -> DeduplicationResult:
        """
        ...
        similarity_duplicates = self.detect_similarity_duplicates(content_items, embeddings)
...
def detect_duplicates(content_items: List[Tuple[str, str]], 
                     embeddings: Optional[Dict[str, List[float]]] = None) -> DeduplicationResult:
