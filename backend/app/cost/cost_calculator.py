...
class CostCalculator:
    ...
    def calculate_operation_cost(
        ...
        if token_count.input_tokens < 0 or token_count.output_tokens < 0:
            raise ValueError(
                f"Token counts cannot be negative: input={token_count.input_tokens}, output={token_count.output_tokens}"
            )
        ...
        input_cost = Decimal(str(token_count.input_tokens / 1000)) * Decimal(
            str(model_config.input_cost_per_1k)
        )
        output_cost = Decimal(str(token_count.output_tokens / 1000)) * Decimal(
            str(model_config.output_cost_per_1k)
        )
    ...
    def calculate_batch_cost(
        ...
        for i, token_count in enumerate(token_counts):
            ...
            breakdown = self.calculate_operation_cost(
                token_count, category, operation_id
            )
    ...
    def check_budget_limits(
        self, proposed_cost: Decimal, session_id: Optional[str] = None
    ) -> Tuple[bool, List[str]]:
        ...
        if self.budget_config.per_operation_limit:
            if proposed_cost > self.budget_config.per_operation_limit:
                allowed = False
                warnings.append(
                    f"Operation cost ${proposed_cost} exceeds per-operation limit ${self.budget_config.per_operation_limit}"
                )
        ...
        if self.budget_config.daily_limit:
            ...
            if projected_daily > self.budget_config.daily_limit:
                if self.budget_config.hard_stop_enabled:
                    grace_limit = self.budget_config.daily_limit * (
                        1 + Decimal(str(self.budget_config.grace_percentage))
                    )
                    if projected_daily > grace_limit:
                        allowed = False
                        warnings.append(
                            f"Daily cost limit exceeded: ${projected_daily} > ${grace_limit} (with grace)"
                        )
                    else:
                        warnings.append(
                            f"Daily cost limit exceeded but within grace: ${projected_daily}"
                        )
                else:
                    warnings.append(
                        f"Daily cost limit exceeded: ${projected_daily} > ${self.budget_config.daily_limit}"
                    )
    ...
    def estimate_document_processing_cost(
        self, document_content: str, processing_types: List[str], models: List[str]
        ...
        for model in models:
            ...
            for processing_type in processing_types:
                ...
                # Calculate cost
                breakdown = self.calculate_operation_cost(
                    token_count, category, f"estimate_{model}_{processing_type}"
                )
    ...
    def optimize_model_selection(
        ...
        if not model_evaluations or model_evaluations[0]["cost_effectiveness"] == 0:
            ...
            suitable_models = [
                m for m in model_evaluations if m["quality"] >= quality_threshold
            ]
            if suitable_models:
    ...
    def generate_cost_forecast(
        self, period: BudgetPeriod, confidence_threshold: float = 0.7
        ...
        variance = sum(
            (float(c.total_cost) - (total_cost / len(recent_costs))) ** 2
            for c in recent_costs
        ) / len(recent_costs)
        confidence = max(
            0.0, min(1.0, 1.0 - (variance / (total_cost / len(recent_costs))))
        )
    ...
    def get_cost_analytics(
        ...
        relevant_costs = [
            c for c in self.cost_history if start_date <= c.timestamp <= end_date
        ]
        ...
        for cost in relevant_costs:
            ...
            if cat not in category_breakdown:
                category_breakdown[cat] = {"cost": Decimal("0"), "operations": 0}
        ...
        return {
            ...
            "totals": {
                ...
                "avg_cost_per_1k_tokens": (
                    float(total_cost / (total_tokens / 1000)) if total_tokens > 0 else 0
                ),
            },
            "models": {
                model: {
            ...
            "daily_costs": {day: float(cost) for day, cost in daily_breakdown.items()},
        }

    def _get_category_multiplier(
        self, category: CostCategory, provider: ModelProvider
    ) -> float:
    ...
    def _check_budget_alerts(self, proposed_cost: Decimal) -> None:
        ...
        for alert in self.budget_config.alerts:
            if alert.period == BudgetPeriod.DAILY and self.budget_config.daily_limit:
                threshold = self.budget_config.daily_limit * Decimal(
                    str(alert.threshold_percentage)
                )
                if current_daily + proposed_cost >= threshold and not alert.triggered:
                    ...
                    alert.alert_count += 1
                    logger.warning(
                        f"Budget alert triggered: {alert.name} - Daily spend approaching ${threshold}"
                    )
