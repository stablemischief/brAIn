...
class CostOptimizer:
    ...
    def optimize_operation(
        ...
        for model in available_models:
            ...
            cost_breakdown = self.cost_calculator.calculate_operation_cost(
                token_count, self._operation_type_to_category(operation_type)
            )

            # Get model performance metrics
            performance = self._model_performance.get(
                model, {"quality": 0.7, "speed": 0.7, "cost_efficiency": 0.7}
            )
        ...
        savings_pct = float(
            (max_cost_option["cost"] - best_option["cost"])
            / max_cost_option["cost"]
            * 100
        )
            ...
            alternative_options=[
                {
                    ...
                    "savings_vs_recommended": float(
                        (alt["cost"] - best_option["cost"]) / best_option["cost"] * 100
                    ),
                }
    ...
    def optimize_batch_processing(
        ...
        operations: List[
            Dict[str, Any]
        ],  # [{'content': str, 'type': str, 'priority': int}]
        ...
        savings_pct = (
            float(savings / original_total_cost * 100) if original_total_cost > 0 else 0
        )
    ...
    def forecast_costs(
        ...
        if confidence < confidence_level:
            logger.info(
                f"Forecast confidence ({confidence:.2f}) below threshold ({confidence_level})"
            )
            return None

        return CostForecast(
            period=forecast_period,
            projected_cost=Decimal(str(forecast["projected_cost"])).quantize(
                Decimal("0.01")
            ),
            confidence=confidence,
    ...
    def analyze_cost_trends(self, analysis_period_days: int = 30) -> CostTrend:
    ...
    def analyze_costs(self, costs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        most_expensive_model = (
            max(model_costs.items(), key=lambda x: x[1])[0] if model_costs else None
        )
        if (
            most_expensive_model
            and model_costs[most_expensive_model] > total_cost * 0.5
        ):
            recommendations.append(
                f"Model {most_expensive_model} accounts for >50% of costs - consider alternatives"
            )
    ...
    def get_optimization_insights(self) -> Dict[str, Any]:
        ...
        for model in model_usage:
            avg_cost = (
                model_costs[model] / model_usage[model]
        ...
        if (
            most_used_model
            and model_efficiency[most_used_model]["efficiency_score"] < 0.7
        ):
            recommendations.append(
                {
                    "type": "model_optimization",
                    ...
                    "potential_savings": self._estimate_model_switch_savings(
                        most_used_model
                    ),
                }
            )
    ...
    def _adjust_quality_for_complexity(
        self, base_quality: float, complexity: float, model: str
    ) -> float:
    ...
    def _calculate_optimization_score(
        ...
        if strategy == OptimizationStrategy.MINIMIZE_COST:
            ...
            consistency_bonus = (
                1.0 - complexity * 0.3
            )  # Bonus for handling simple, consistent content
    ...
    def _exponential_smoothing_forecast(
        self, daily_costs: Dict[str, float], period: BudgetPeriod, alpha: float = 0.3
    ...
    def _calculate_forecast_confidence(
        ...
        recent_trend = (
            np.mean(costs[-5:]) / np.mean(costs[:-5]) if len(costs) >= 10 else 1.0
        )
    ...
    def _generate_optimization_rationale(
        ...
        if strategy == OptimizationStrategy.MINIMIZE_COST:
            rationale_parts.append(
                f"Selected {model} as the most cost-effective option at ${cost:.4f}"
            )
        elif strategy == OptimizationStrategy.MAXIMIZE_QUALITY:
            rationale_parts.append(
                f"Selected {model} for highest quality score ({quality:.2f})"
            )
        elif strategy == OptimizationStrategy.BALANCED:
            rationale_parts.append(
                f"Selected {model} for optimal balance of cost (${cost:.4f}) and quality ({quality:.2f})"
            )

        # Complexity consideration
        if complexity > 0.7:
            rationale_parts.append(
                "High content complexity detected, favoring more capable models"
            )
        elif complexity < 0.3:
            rationale_parts.append(
                "Low complexity content allows for cost optimization"
            )
    ...
    def _calculate_recommendation_confidence(
        self, evaluations: List[Dict[str, Any]], complexity: float
    ) -> float:
        ...
        return (
            ...
            + options_confidence * 0.2
        )

    def _estimate_batch_processing_time(
        self, operations: List[Dict[str, Any]], assignments: Dict[str, str]
    ) -> float:
    ...
    def _linear_regression_forecast(
        self, daily_costs: Dict[str, float], period: BudgetPeriod
    ) -> Optional[Dict[str, Any]]:
    ...
    def _moving_average_forecast(
        self, daily_costs: Dict[str, float], period: BudgetPeriod, window: int = 7
    ) -> Optional[Dict[str, Any]]:
