...
class OptimizationStrategy(BaseModel):
    ...
    recommended_chunk_size: int = Field(
        default=1000, description="Recommended chunk size"
    )
    ...
    postprocessing_steps: List[str] = Field(
        default_factory=list, description="Postprocessing steps"
    )
    parameters: Dict[str, Any] = Field(
        default_factory=dict, description="Strategy parameters"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Strategy metadata"
    )
...
class OptimizationResult(BaseModel):
    ...
    memory_usage_reduced: float = Field(
        ge=0.0, description="Memory usage reduction percentage"
    )
    quality_impact: float = Field(
        ge=-1.0, le=1.0, description="Quality impact (-1 to 1)"
    )
    recommendations: List[str] = Field(
        default_factory=list, description="Additional recommendations"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Optimization metadata"
    )
...
class FormatOptimizationEngine:
    ...
    async def select_strategy(
        ...
        level_enum = (
            ...
            if optimization_level in OptimizationLevel
        ...
        best_strategy = max(
            level_strategies,
            key=lambda s: self._calculate_strategy_score(
                s, detected_format, confidence_score
            ),
        )
    ...
    async def apply_optimizations(
        ...
        try:
            ...
            time_saved = max(
                0.0,
                preprocessing_result.get("time_saved", 0.0)
            ...
            memory_saved = max(
                0.0,
                preprocessing_result.get("memory_saved", 0.0)
    ...
    async def _apply_preprocessing(
        self, file_path: Path, strategy: OptimizationStrategy, file_classification: Any
    ) -> Dict[str, Any]:
        """Apply preprocessing optimizations."""
        optimizations = []
        time_saved = 0.0
        ...
        for step in strategy.preprocessing_steps:
            try:
                if step == "file_size_check":
                    ...
                    result = await self._optimize_format_validation(
                        file_path, file_classification
                    )
    ...
    async def _apply_format_optimizations(
        ...
        # Structured data optimizations
            ...
            result = await self._optimize_structured_data_processing(
                file_path, strategy
            )
            optimizations.extend(result["optimizations"])
            time_saved += result.get("time_saved", 0.0)
    ...
    async def _apply_performance_optimizations(
        self, file_path: Path, strategy: OptimizationStrategy, processing_context: Any
    ) -> Dict[str, Any]:
        """Apply performance optimizations."""
        optimizations = []
        time_saved = 0.0
    ...
    async def _optimize_pdf_processing(
        self, file_path: Path, strategy: OptimizationStrategy
    ) -> Dict[str, Any]:
    ...
    async def _optimize_office_processing(
        self, file_path: Path, strategy: OptimizationStrategy
    ) -> Dict[str, Any]:
    ...
    async def _optimize_text_processing(
        self, file_path: Path, strategy: OptimizationStrategy
    ) -> Dict[str, Any]:
    ...
    async def _optimize_image_processing(
        self, file_path: Path, strategy: OptimizationStrategy
    ) -> Dict[str, Any]:
        ...
        return {"optimizations": optimizations, "time_saved": time_saved}

    async def _optimize_structured_data_processing(
        self, file_path: Path, strategy: OptimizationStrategy
    ) -> Dict[str, Any]:
    ...
    async def _optimize_file_size_check(
        self, file_path: Path, strategy: OptimizationStrategy
    ) -> Dict[str, Any]:
    ...
    async def _optimize_content_sampling(
        self, file_path: Path, strategy: OptimizationStrategy
    ) -> Dict[str, Any]:
    ...
    async def _optimize_format_validation(
        self, file_path: Path, file_classification: Any
    ) -> Dict[str, Any]:
    ...
    async def _optimize_chunking_strategy(
        self, file_path: Path, strategy: OptimizationStrategy
    ) -> Dict[str, Any]:
    ...
    async def _generate_optimization_recommendations(
        ...
        if (
            detected_format == "pdf"
            and "pdf_page_sampling" not in applied_optimizations
        ):
            recommendations.append("Consider page sampling for large PDF files")

        if (
            detected_format in ["docx", "doc"]
            and "office_selective_extraction" not in applied_optimizations
        ):
            recommendations.append(
                "Enable selective content extraction for Office documents"
            )

        # General recommendations
        if confidence < 0.7:
            recommendations.append(
                "Low format confidence - consider manual format specification"
            )

        if strategy.optimization_level == OptimizationLevel.SPEED:
            recommendations.append(
                "For better quality, consider using 'balanced' or 'quality' optimization"
            )

        if not strategy.parallel_processing:
            recommendations.append(
                "Enable parallel processing for better performance on large files"
            )
    ...
    def _get_generic_strategy(self, optimization_level: str) -> OptimizationStrategy:
        ...
        return OptimizationStrategy(
            ...
            parallel_processing=level_enum
            in [OptimizationLevel.BALANCED, OptimizationLevel.QUALITY],
...
async def optimize_file_processing(
    file_path: Path, detected_format: str, optimization_level: str = "balanced"
) -> OptimizationResult:
