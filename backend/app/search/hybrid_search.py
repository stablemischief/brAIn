...
class HybridSearchEngine:
    ...
    async def search(
        ...
        try:
            ...
            final_strategy = strategy or self._select_strategy(
    ...
    def _select_strategy(
        self, query_analysis: QueryAnalysis, user_context: Optional[Dict[str, Any]]
    ) -> SearchStrategy:
    ...
    async def _execute_search(
        ...
        if strategy == SearchStrategy.VECTOR_ONLY:
            ...
            SearchStrategy.VECTOR_FIRST,
            SearchStrategy.KEYWORD_FIRST,
        ]:
            ...
            return await self._adaptive_search(
                query_analysis, filters, limit, user_context
            )

        else:
            # Fallback to hybrid balanced
            return await self._hybrid_search(
                query_analysis, SearchStrategy.HYBRID_BALANCED, filters, limit
            )
    ...
    async def _vector_search(
        ...
        try:
            ...
            params = (
                [query_embedding, self.config.vector_similarity_threshold]
                + filter_params
                + [limit]
            )
    ...
    async def _adaptive_search(
        ...
        strategies_to_try = [
            ...
            (
                SearchStrategy.VECTOR_FIRST
                if query_analysis.is_semantic_query
                else SearchStrategy.KEYWORD_FIRST
            ),
        ]
    ...
    def _reciprocal_rank_fusion(
        ...
        for rank, result in enumerate(vector_results, 1):
            ...
            doc_scores[doc_id] = (
                doc_scores.get(doc_id, 0) + rrf_score * self.config.vector_weight
            )
        ...
        for rank, result in enumerate(keyword_results, 1):
            ...
            doc_scores[doc_id] = (
                doc_scores.get(doc_id, 0) + rrf_score * self.config.keyword_weight
            )
            ...
            if doc_id in doc_results:
                ...
                doc_results[doc_id].ranking_factors["keyword_rrf"] = (
                ...
                if not result.ranking_factors:
                    result.ranking_factors = {}
                result.ranking_factors["keyword_rrf"] = (
                    rrf_score * self.config.keyword_weight
                )
    ...
    def _calculate_bm25_score(
        self, query_tokens: List[str], document: str, base_score: float
    ) -> float:
        ...
        for token in query_tokens:
            ...
            denominator = tf + self.config.bm25_k1 * (
                1 - self.config.bm25_b + self.config.bm25_b * doc_length / avgdl
            )
        ...
        for key, value in filters.items():
            if key == "document_type":
                ...
                if isinstance(value, dict) and "start" in value and "end" in value:
                    conditions.append(
                        f"AND d.created_at BETWEEN ${param_count} AND ${param_count + 1}"
                    )
                ...
                if isinstance(value, dict):
                    for meta_key, meta_value in value.items():
                        conditions.append(
                            f"AND d.metadata->>'{meta_key}' = ${param_count}"
                        )
...
def initialize_hybrid_search_engine(
    config: HybridSearchConfig, db_pool: asyncpg.Pool
) -> HybridSearchEngine:
