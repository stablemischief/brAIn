...
class Settings(BaseSettings):
    ...
    session_max_age: int = Field(
        default=86400, description="Session max age in seconds"
    )

    # JWT Authentication Settings
    jwt_secret: Optional[str] = Field(
        default=None, description="JWT signing secret key"
    )
    jwt_algorithm: str = Field(default="HS256", description="JWT signing algorithm")
    jwt_expiry_minutes: int = Field(
        default=1440, description="JWT token expiry in minutes (24 hours)"
    )
    ...
    allow_api_key_auth: bool = Field(
        default=True, description="Allow API key authentication"
    )
    admin_api_key: Optional[str] = Field(
        default=None, description="Admin API key for system access"
    )
    ...
    openai_max_tokens: int = Field(
        default=4000, description="Max tokens per OpenAI request"
    )
    openai_temperature: float = Field(
        default=0.1, description="OpenAI temperature setting"
    )
    ...
    langfuse_enabled: bool = Field(
        default=True, description="Enable Langfuse monitoring"
    )
    ...
    redis_max_connections: int = Field(
        default=10, description="Redis max connection pool size"
    )
    ...
    max_concurrent_tasks: int = Field(
        default=5, description="Max concurrent processing tasks"
    )
    ...
    similarity_threshold: float = Field(
        default=0.8, description="Vector similarity threshold"
    )
    ...
    default_monthly_budget: float = Field(
        default=100.0, description="Default monthly budget in USD"
    )
    cost_alert_threshold: float = Field(
        default=0.8, description="Cost alert threshold (80% of budget)"
    )
            ...
            if environment == "production":
                ...
                for field in required_fields:
                    if not values.get(field):
                        raise ValueError(
                            f"{field} is required in production environment"
                        )
