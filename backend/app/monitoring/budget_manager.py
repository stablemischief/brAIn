...
class BudgetAlert(BaseModel):
    ...
    user_id: Optional[UUID] = Field(
        default=None,
        description="User ID for user-specific alerts"
    )
    ...
    threshold_percentage: float = Field(
        description="Budget percentage threshold (0.0-1.0)",
        ge=0.0,
        le=1.0
    )
    
    budget_period: BudgetPeriod = Field(
        description="Budget period for the alert"
    )
...
class BudgetStatus(BaseModel):
    ...
    budget_limit: float = Field(
        description="Budget limit for the period",
        ge=0.0
    )
    
    current_spending: float = Field(
        description="Current spending in the period",
        ge=0.0
    )
    
    projected_spending: Optional[float] = Field(
        default=None,
        description="Projected spending for the full period"
    )
    
    remaining_budget: float = Field(
        description="Remaining budget for the period"
    )
    ...
    days_remaining: int = Field(
        description="Days remaining in the period",
        ge=0
    )
    
    is_over_budget: bool = Field(
        description="Whether currently over budget"
    )
...
class SpendingForecast(BaseModel):
    ...
    projected_spending: float = Field(
        description="Projected total spending",
        ge=0.0
    )
    ...
    days_to_budget_exhaustion: Optional[int] = Field(
        default=None,
        description="Days until budget is exhausted (if trending over)"
    )
...
class BudgetManager:
    ...
    def set_user_budget(
        ...
        try:
            ...
            logger.info(f"Set {period.value} budget limit of ${budget_limit} for user {user_id}")
            return True
    ...
    def get_budget_status(
        ...
        percentage_used = min(current_spending / budget_limit if budget_limit > 0 else 0, 1.0)
    ...
    def check_budget_alerts(
        ...
        for alert in self._alerts.values():
            ...
            if percentage_used >= alert.threshold_percentage:
                ...
                if last_triggered and (datetime.utcnow() - last_triggered).seconds < 3600:
    ...
    def get_cost_optimization_recommendations(
        ...
        if spending_analysis.get("expensive_models_usage", 0) > 0.3:
            recommendations.append(CostOptimizationRecommendation(
                ...
                potential_savings=spending_analysis.get("model_switching_savings", 50.0),
        ...
        if spending_analysis.get("average_tokens_per_request", 0) > 2000:
            recommendations.append(CostOptimizationRecommendation(
                ...
                potential_savings=spending_analysis.get("token_optimization_savings", 20.0),
                implementation_effort="low",
                priority=5
            ))
    ...
    def _analyze_spending_trend(self, daily_spending: List[float]) -> str:
        ...
        recent_avg = sum(daily_spending[-3:]) / 3 if len(daily_spending) >= 3 else daily_spending[-1]
        earlier_avg = sum(daily_spending[:-3]) / (len(daily_spending) - 3) if len(daily_spending) > 3 else daily_spending[0]
