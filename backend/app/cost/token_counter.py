...
class TokenCounter:
    ...
    def count_tokens_messages(
        ...
        return TokenCount(
            ...
            provider=config.provider,
            encoding_name=config.encoding,
            metadata={"message_count": len(messages), "has_functions": bool(functions)},
        )
    ...
    def count_tokens_by_provider(
        self, text: str, model: str, provider: Optional[ModelProvider] = None
    ) -> TokenCount:
    ...
    def validate_context_length(
        self, token_count: TokenCount, model: str
        ...
        if token_count.total_tokens > config.context_window:
            return (
                False,
                f"Token count {token_count.total_tokens} exceeds model {model} context window of {config.context_window}",
            )
    ...
    def batch_count_tokens(
        self, texts: List[str], model: str = "gpt-4"
    ) -> List[TokenCount]:
    ...
    def estimate_processing_tokens(
        self, document_content: str, processing_type: str, model: str = "gpt-4"
    ) -> TokenCount:
        """
        ...
        if processing_type in ["summarization", "extraction", "classification"]:
            output_estimate = min(
                1000, base_tokens.total_tokens // 4
            )  # 25% of input, max 1000
        ...
        return TokenCount(
            ...
            model=model,
            provider=self.model_configs.get(
                model, self.model_configs["gpt-4"]
            ).provider,
            metadata={
...
def estimate_processing_cost(
    document_content: str, processing_type: str, model: str = "gpt-4"
) -> Tuple[TokenCount, float]:
    """Estimate tokens and cost for document processing"""
    counter = get_token_counter()
    token_count = counter.estimate_processing_tokens(
