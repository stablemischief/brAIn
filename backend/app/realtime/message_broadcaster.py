...
class MessageFilter:
    ...
    def should_deliver(
        self, message: BroadcastMessage, connection_info: ConnectionInfo
    ) -> bool:
...
class UserScopeFilter(MessageFilter):
    ...
    def should_deliver(
        ...
        if message.scope == MessageScope.USER:
            return (
                str(connection_info.user_id) == message.target_id
            ...
            return message.target_id in (
                connection_info.metadata.get("roles", [])
        ...
        if self.redis_client:
            try:
                await self.redis_client.lpush(
                    "broadcast_messages", json.dumps(message.dict(), default=str)
                )
                # Set TTL on Redis list
                if message.ttl_seconds:
                    await self.redis_client.expire(
                        "broadcast_messages", message.ttl_seconds
                    )
            except Exception as e:
    ...
    async def _deliver_to_connection(
        ...
        try:
            await self.websocket_manager.send_to_connection(
                connection_id, json.dumps(message_data)
            )
        except Exception as e:
            logger.warning(
                f"Failed to deliver message to connection {connection_id}: {e}"
            )
            raise

    def _should_deliver_message(
        self, message: BroadcastMessage, connection_info: ConnectionInfo
    ) -> bool:
