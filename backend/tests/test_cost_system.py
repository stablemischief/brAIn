...
def budget_manager():
    ...
    config = BudgetConfig(
        daily_limit=10.0, monthly_limit=100.0, alert_threshold=0.8, enforce_limits=True
    )
...
class TestTokenCounter:
    ...
    def test_count_tokens_openai(self, token_counter, sample_text):
        ...
        result = token_counter.count_tokens(
            text=sample_text, model="gpt-4", provider=ModelProvider.OPENAI
        )
    ...
    def test_count_tokens_anthropic(self, token_counter, sample_text):
        ...
        result = token_counter.count_tokens(
            text=sample_text, model="claude-3-opus", provider=ModelProvider.ANTHROPIC
        )
    ...
    def test_estimate_tokens(self, token_counter, long_text):
        ...
        actual = token_counter.count_tokens(
            long_text, model="gpt-4", provider=ModelProvider.OPENAI
        )
    ...
    def test_unsupported_model(self, token_counter):
        ...
        with pytest.raises(ValueError, match="Unsupported model"):
            token_counter.count_tokens(
                "test text", model="unsupported-model", provider=ModelProvider.CUSTOM
            )

    def test_empty_text(self, token_counter):
        """Test token counting with empty text"""
        result = token_counter.count_tokens(
            text="", model="gpt-4", provider=ModelProvider.OPENAI
        )
    ...
    def test_token_truncation(self, token_counter):
        ...
        result = token_counter.count_tokens_with_truncation(
            text=very_long_text, model="gpt-4", max_tokens=1000
        )
...
class TestCostCalculator:
    ...
    def test_update_pricing(self, cost_calculator):
        ...
        new_pricing = {"input_per_1k": 0.05, "output_per_1k": 0.10}
    ...
    def test_cost_with_multiplier(self, cost_calculator):
        ...
        premium_cost = cost_calculator.calculate_cost(token_count, multiplier=1.5)
    ...
    def test_cost_alert_trigger(self, cost_calculator):
        ...
        with patch.object(cost_calculator, "trigger_alert") as mock_alert:
            cost = cost_calculator.calculate_cost(
                token_count, alert_threshold=0.01  # Low threshold to trigger
            )
...
class TestCostOptimizer:
    ...
    def test_recommend_model_switch(self, cost_optimizer):
        ...
        assert any(
            r.strategy == OptimizationStrategy.MODEL_SWITCH for r in recommendations
        )
...
class TestCostAnalytics:
    ...
    def test_generate_daily_report(self, cost_analytics):
        ...
        report = cost_analytics.generate_report(
            period=AggregationPeriod.DAILY, date=datetime.now()
        )
    ...
    def test_identify_usage_patterns(self, cost_analytics):
        ...
        usage_logs = [{"hour": i, "tokens": 1000 * (1 + i % 3)} for i in range(24)]
    ...
    def test_forecast_costs(self, cost_analytics):
        ...
        forecast = cost_analytics.forecast_costs(
            historical_data=historical, days_ahead=7
        )
...
class TestCostSystemIntegration:
    ...
    async def test_end_to_end_cost_tracking(
        self, token_counter, cost_calculator, budget_manager, cost_analytics
    ):
        ...
        token_count = token_counter.count_tokens(
            text=text, model="gpt-4", provider=ModelProvider.OPENAI
        )
    ...
    @pytest.mark.asyncio
    async def test_optimization_workflow(self, cost_optimizer, cost_analytics):
    ...
    def test_budget_enforcement_integration(
        self, budget_manager, cost_calculator, token_counter
    ):
...
class TestErrorHandling:
    ...
    def test_invalid_model_handling(self, token_counter):
        ...
        with pytest.raises(ValueError):
            token_counter.count_tokens(
                "test", model="invalid-model", provider=ModelProvider.OPENAI
            )
...
class TestPerformance:
    ...
    def test_token_counting_performance(self, token_counter):
        ...
        start = time.time()
        result = token_counter.count_tokens(
            large_text, model="gpt-4", provider=ModelProvider.OPENAI
        )
