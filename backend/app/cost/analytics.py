...
class CostAnalytics:
    ...
    def generate_report(
        ...
        **kwargs,
        ...
        if report_type == ReportType.DAILY_SUMMARY:
            ...
            return self._generate_model_performance_report(
            ...
            return self._generate_optimization_insights_report(
                report_id, start_date, end_date
            )
    ...
    def get_real_time_metrics(self) -> Dict[str, Any]:
        ...
        today_costs = [
            c for c in self.cost_calculator.cost_history if c.timestamp >= today_start
        ]
        ...
        week_costs = [
            c for c in self.cost_calculator.cost_history if c.timestamp >= week_start
        ]
        ...
        month_costs = [
            c for c in self.cost_calculator.cost_history if c.timestamp >= month_start
        ]
        ...
        recent_costs = [
            c for c in self.cost_calculator.cost_history if c.timestamp >= hour_ago
        ]
        ...
        yesterday_costs = [
            c
            for c in self.cost_calculator.cost_history
        ...
        return {
            ...
            "operations": {
                ...
                "avg_cost_per_operation": (
                    float(today_total / today_operations) if today_operations > 0 else 0
                ),
            },
            "trends": {
                "daily_trend": daily_trend,
                "daily_change_pct": (
                    float((today_total - yesterday_total) / yesterday_total * 100)
                    if yesterday_total > 0
                    else 0
                ),
            },
            "model_distribution": dict(model_usage),
            "alerts": {"total_active": active_alerts, "critical": critical_alerts},
        }
    ...
    def detect_cost_anomalies(self, lookback_days: int = 7) -> List[CostAlert]:
        ...
        recent_costs = [
            c for c in self.cost_calculator.cost_history if c.timestamp >= cutoff
        ]
        ...
        if len(daily_amounts) >= 3:
            ...
            std_cost = (
                statistics.stdev(daily_amounts)
            ...
            if today_cost > mean_cost * Decimal(
                str(self.thresholds["cost_spike_multiplier"])
            ):
                alert = CostAlert(
                    ...
                    threshold_value=float(
                        mean_cost
                        * Decimal(str(self.thresholds["cost_spike_multiplier"]))
                    ),
        ...
        if self.optimizer:
            ...
            for model, efficiency_data in insights.get("model_efficiency", {}).items():
                if (
                    ...
                    < self.thresholds["efficiency_threshold"]
                ):
                    alert = CostAlert(
        ...
        if self.budget_enforcer:
            ...
            for rule in budget_status.get("rules", []):
                if rule["percentage"] >= self.thresholds[
                    "budget_warning_threshold"
                ] * 100 and rule["status"] not in ["ok", "caution"]:
                    ...
                    alert = CostAlert(
                        ...
                        threshold_value=self.thresholds["budget_warning_threshold"]
                        * 100,
    ...
    def _generate_daily_summary(
        ...
        summary = {
            ...
            "average_cost_per_operation": (
                float(total_cost / total_operations) if total_operations > 0 else 0
            ),
            "most_used_model": (
                max(model_breakdown, key=model_breakdown.get)
                if model_breakdown
                else None
            ),
            "peak_hour": (
                max(hourly_breakdown, key=hourly_breakdown.get)
                if hourly_breakdown
                else None
            ),
        }
        ...
        detailed_analysis = {
            ...
            "hourly_distribution": {
                str(k): float(v) for k, v in hourly_breakdown.items()
            },
        }
        ...
        if total_operations > 0:
            insights.append(
                f"Processed {total_operations} operations with total cost of ${total_cost:.4f}"
            )

            if model_breakdown:
                top_model = max(model_breakdown, key=model_breakdown.get)
                insights.append(
                    f"Most used model: {top_model} (${model_breakdown[top_model]:.4f})"
                )

            if hourly_breakdown:
                peak_hour = max(hourly_breakdown, key=hourly_breakdown.get)
                insights.append(
                    f"Peak activity hour: {peak_hour}:00 (${hourly_breakdown[peak_hour]:.4f})"
                )
    ...
    def _generate_optimization_insights_report(
        self, report_id: str, start_date: datetime, end_date: datetime
    ) -> AnalyticsReport:
    ...
    def _analyze_cost_period(self, costs: List[CostBreakdown]) -> Dict[str, Any]:
        ...
        return {
            "total_cost": float(total_cost),
            "daily_average": (
                statistics.mean(daily_totals.values()) if daily_totals else 0
            ),
            "peak_day": (
                max(daily_totals, key=daily_totals.get) if daily_totals else None
            ),
            "lowest_day": (
                min(daily_totals, key=daily_totals.get) if daily_totals else None
            ),
            "cost_volatility": (
                statistics.stdev(daily_totals.values()) if len(daily_totals) > 1 else 0
            ),
        }
    ...
    def _analyze_model_usage(self, costs: List[CostBreakdown]) -> Dict[str, Any]:
        ...
        model_stats = defaultdict(
            lambda: {"count": 0, "total_cost": Decimal("0"), "total_tokens": 0}
        )
    ...
    def _analyze_category_distribution(
        ...
        category_stats = defaultdict(lambda: {"count": 0, "total_cost": Decimal("0")})
        ...
        for category, stats in category_stats.items():
            percentage = (
                float(stats["total_cost"] / total_cost * 100) if total_cost > 0 else 0
            )
            ...
            category_insights[category] = {
                ...
                "average_cost": (
                    float(stats["total_cost"] / stats["count"])
                    if stats["count"] > 0
                    else 0
                ),
            }
    ...
    def _identify_optimization_opportunities(
        ...
        if len(sorted_costs) >= 5:
            ...
            if (
                top_5_cost / total_cost > 0.5
            ):  # Top 5 operations account for >50% of cost
    ...
    def _generate_model_performance_report(
        self, report_id: str, start_date: datetime, end_date: datetime
    ) -> AnalyticsReport:
    ...
    def _generate_budget_status_report(
        self, report_id: str, start_date: datetime, end_date: datetime
    ) -> AnalyticsReport:
