...
class ContentAnalysis(BaseModel):
    ...
    metadata_richness: float = Field(
        ge=0.0, le=1.0, description="Metadata richness score"
    )
...
class AIFileClassifier:
    ...
    async def classify_with_content_analysis(
        self, file_path: Union[str, Path], domain_context: Optional[str] = None
    ) -> FileTypeAnalysis:
        """
        ...
        try:
            ...
            content_analysis = await self._analyze_file_content(
                file_path, domain_context
            )

            # Step 3: AI-powered analysis (if available)
            ai_analysis = {}
            if self.ai_client:
                ai_analysis = await self._ai_content_analysis(
    ...
    async def _analyze_file_content(
        self, file_path: Path, domain_context: Optional[str]
    ) -> ContentAnalysis:
        ...
        try:
            ...
            if not content:
                ...
                return ContentAnalysis(
                    structure_type="binary", complexity_score=0.0, metadata_richness=0.0
                )
            ...
            return ContentAnalysis(
                structure_type="unknown", complexity_score=0.0, metadata_richness=0.0
            )

    async def _ai_content_analysis(
        self, file_path: Path, content_analysis: ContentAnalysis
    ) -> Dict[str, Any]:
    ...
    async def _combine_analysis_results(
        ...
        if content_analysis.content_patterns:
            ...
            candidates.append(
                {
                    "format": best_pattern,
                    "confidence": (len(content_analysis.content_patterns) / 3)
                    * content_weight,
    ...
    async def _generate_processing_recommendations(
        ...
        if classification["confidence"] < 0.5:
            recommendations.append(
                "Low confidence - consider manual format specification"
            )

        # Content complexity recommendations
        if content_analysis.complexity_score > 0.8:
            recommendations.append(
                "High complexity content - use quality-focused processing"
            )
        elif content_analysis.complexity_score < 0.3:
            recommendations.append(
                "Simple content - speed-optimized processing suitable"
            )
    ...
    def _calculate_metadata_richness(self, content: str, patterns: List[str]) -> float:
        ...
        structural_elements = len(
            re.findall(r"^#+\s|^\*\s|^\d+\.\s|^-\s", content, re.MULTILINE)
        )
        richness_factors += min(structural_elements / 20.0, 1.0) * 0.3

        # Formatting indicators
        formatting_indicators = len(
            re.findall(r"\*\*[^*]+\*\*|__[^_]+__|`[^`]+`", content)
        )
...
async def classify_file_ai(
    file_path: Union[str, Path], domain_context: Optional[str] = None
) -> FileTypeAnalysis:
