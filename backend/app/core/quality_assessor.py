...
class ContentQualityMetrics(BaseModel):
    ...
    coherence_score: float = Field(
        ge=0.0, le=1.0, description="Content coherence score"
    )
    information_density: float = Field(
        ge=0.0, le=1.0, description="Information density score"
    )
    language_consistency: float = Field(
        ge=0.0, le=1.0, description="Language consistency score"
    )
    entity_coverage: float = Field(ge=0.0, le=1.0, description="Entity coverage score")
    structure_quality: float = Field(
        ge=0.0, le=1.0, description="Document structure quality"
    )
...
class ExtractionQualityMetrics(BaseModel):
    ...
    extraction_accuracy: float = Field(
        ge=0.0, le=1.0, description="Extraction accuracy estimate"
    )
    text_preservation: float = Field(
        ge=0.0, le=1.0, description="Text preservation quality"
    )
    formatting_retention: float = Field(
        ge=0.0, le=1.0, description="Formatting retention score"
    )
    error_rate: float = Field(ge=0.0, le=1.0, description="Estimated error rate")
    method_reliability: float = Field(
        ge=0.0, le=1.0, description="Extraction method reliability"
    )
...
class ProcessingQualityMetrics(BaseModel):
    ...
    embedding_reliability: float = Field(
        ge=0.0, le=1.0, description="Embedding generation quality"
    )
    metadata_completeness: float = Field(
        ge=0.0, le=1.0, description="Metadata completeness"
    )
    cost_efficiency: float = Field(ge=0.0, le=1.0, description="Cost efficiency score")
    processing_speed: float = Field(
        ge=0.0, le=1.0, description="Processing speed score"
    )
...
class QualityAssessmentResult(BaseModel):
    ...
    issues_detected: List[str] = Field(
        default_factory=list, description="Issues found during assessment"
    )
...
class QualityAssessmentEngine:
    ...
    def calculate_language_consistency(self, text: str) -> float:
        ...
        try:
            if not LANGDETECT_AVAILABLE:
                ...
                english_ratio = (
                    english_words_count / total_words if total_words > 0 else 0
                )
    ...
    def assess_extraction_quality(
        ...
        error_count = (
            len(processing_quality.errors) + len(processing_quality.warnings) * 0.5
        )
    ...
    def assess_processing_quality(
        self, chunks: List[TextChunk], processing_time: float, estimated_cost: Decimal
        ...
        if not chunks:
            ...
            size_variance = (
                statistics.variance(chunk_sizes) if len(chunk_sizes) > 1 else 0
            )
            ...
            avg_chunk_quality = statistics.mean(
                [chunk.quality_score for chunk in chunks]
            )
    ...
    def generate_recommendations(
        ...
        if content_metrics.readability_score < 30:
            recommendations.append(
                "Content has low readability - consider using simpler language"
            )

        if content_metrics.coherence_score < 0.6:
            recommendations.append(
                "Content lacks coherence - add transition words and logical flow"
            )

        if content_metrics.information_density < 0.4:
            recommendations.append(
                "Content has low information density - consider adding more specific details"
            )

        if content_metrics.language_consistency < 0.8:
            recommendations.append(
                "Content has mixed languages - ensure consistent language use"
            )

        # Extraction recommendations
        if extraction_metrics.extraction_accuracy < 0.7:
            recommendations.append(
                "Extraction accuracy is low - consider alternative extraction methods"
            )

        if extraction_metrics.text_preservation < 0.6:
            recommendations.append(
                "Text preservation is poor - check for format-specific extraction issues"
            )

        if extraction_metrics.error_rate > 0.3:
            recommendations.append(
                "High error rate detected - review extraction process for issues"
            )

        # Processing recommendations
        if processing_metrics.chunking_quality < 0.6:
            recommendations.append(
                "Chunking quality is poor - adjust chunk size or overlap settings"
            )

        if processing_metrics.cost_efficiency < 0.5:
            recommendations.append(
                "Processing cost is high - consider optimizing token usage"
            )

        if processing_metrics.processing_speed < 0.5:
            recommendations.append(
                "Processing speed is slow - consider performance optimizations"
            )

        return recommendations

    def assess_processing_result(
        self, result: FileProcessingResult
