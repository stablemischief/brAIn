...
class ConnectionInfo(BaseModel):
    ...
    user_id: Optional[UUID] = Field(
        default=None, description="User ID associated with connection"
    )

    session_id: Optional[str] = Field(
        default=None, description="Session ID for grouping connections"
    )
    ...
    last_ping: Optional[datetime] = Field(
        default=None, description="Last ping received"
    )

    subscriptions: Set[str] = Field(
        default_factory=set, description="Active subscriptions"
    )
...
class WebSocketConnectionManager:
    ...
    async def _handle_connection_failure(self, connection_id: str) -> None:
        ...
        try:
            ...
            if attempts < self.max_reconnect_attempts:
                ...
                logger.info(
                    f"Connection {connection_id} failed, attempt {attempts + 1}/{self.max_reconnect_attempts}"
                )
            else:
                # Max attempts reached, force disconnect
                logger.warning(
                    f"Connection {connection_id} exceeded max reconnection attempts"
                )
                await self.disconnect(
                    connection_id, reason="Max reconnection attempts exceeded"
                )
    ...
    async def _schedule_cleanup(self, connection_id: str) -> None:
        ...
        try:
            ...
            if (
                connection_id in self.connection_states
                and self.connection_states[connection_id] == ConnectionState.ERROR
            ):
    ...
    async def _handle_pong_message(
        self, connection_id: str, data: Dict[str, Any]
    ) -> None:
        ...
        try:
            if connection_id in self.connection_metrics:
                ...
                if "timestamp" in data:
                    ping_timestamp = datetime.fromisoformat(
    ...
    def get_connection_health(self, connection_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "metrics": {
                ...
                "last_ping": (
                    metrics.last_ping.isoformat() if metrics.last_ping else None
                ),
                "last_pong": (
                    metrics.last_pong.isoformat() if metrics.last_pong else None
                ),
