...
class RuleExecutionResult(BaseModel):
    ...
    extracted_content: List[str] = Field(default_factory=list, description="Extracted content")
...
class RuleSet(BaseModel):
    ...
    domain: Optional[str] = Field(None, description="Domain context (legal, medical, etc.)")
    ...
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Ruleset metadata")
...
class ExtractionRulesEngine:
    ...
    async def compile_rules(self, rule_definitions: List[Union[str, Dict[str, Any], CustomRule]]) -> RuleSet:
        """
    ...
    async def extract_with_rules(
        ...
        try:
            ...
            # Process results
            final_content = await self._process_execution_results(execution_results, content)
    ...
    async def _execute_ruleset(self, job: ExtractionJob, content: str) -> List[RuleExecutionResult]:
    ...
    async def _execute_single_rule(self, rule: CustomRule, content: str) -> RuleExecutionResult:
        ...
        try:
            if rule.rule_type == RuleType.REGEX:
                ...
                return await self._execute_transformation_rule(rule, content, start_time)
    ...
    async def _execute_regex_rule(self, rule: CustomRule, content: str, start_time: datetime) -> RuleExecutionResult:
        ...
        try:
            ...
            return RuleExecutionResult(
                ...
                execution_time=execution_time,
                metadata={"pattern": rule.pattern, "flags": "IGNORECASE|MULTILINE|DOTALL"}
            )
    ...
    async def _execute_pattern_rule(self, rule: CustomRule, content: str, start_time: datetime) -> RuleExecutionResult:
    ...
    async def _execute_conditional_rule(self, rule: CustomRule, content: str, start_time: datetime) -> RuleExecutionResult:
    ...
    async def _execute_transformation_rule(self, rule: CustomRule, content: str, start_time: datetime) -> RuleExecutionResult:
    ...
    async def _execute_validation_rule(self, rule: CustomRule, content: str, start_time: datetime) -> RuleExecutionResult:
        ...
        try:
            ...
            if 'min_length' in rule.conditions:
                if len(content) < rule.conditions['min_length']:
                    ...
                    validation_messages.append(f"Content too short: {len(content)} < {rule.conditions['min_length']}")
            
            if 'max_length' in rule.conditions:
                if len(content) > rule.conditions['max_length']:
                    validation_passed = False
                    validation_messages.append(f"Content too long: {len(content)} > {rule.conditions['max_length']}")
            ...
            if rule.pattern:
                ...
                if not pattern.search(content):
                    validation_passed = False
                    validation_messages.append(f"Required pattern not found: {rule.pattern}")
            ...
            return RuleExecutionResult(
                ...
                extracted_content=validation_messages if not validation_passed else ["Validation passed"],
                execution_time=execution_time,
    ...
    async def _execute_json_path_rule(self, rule: CustomRule, content: str, start_time: datetime) -> RuleExecutionResult:
    ...
    async def _execute_xpath_rule(self, rule: CustomRule, content: str, start_time: datetime) -> RuleExecutionResult:
    ...
    async def _process_execution_results(self, results: List[RuleExecutionResult], original_content: str) -> str:
