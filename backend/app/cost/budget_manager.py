...
class BudgetEnforcer:
    ...
    def __init__(
        ...
        self.spending_cache: Dict[str, Dict[str, Decimal]] = (
            {}
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                """,
                    (
                        ...
                        float(rule.limit),
                        (
                            json.dumps([c.value for c in rule.categories])
                            if rule.categories
                            else None
                        ),
                        json.dumps(rule.models) if rule.models else None,
                        rule.priority,
                        rule.enforcement_action.value,
    ...
    def check_budget_compliance(
        self, cost_breakdown: CostBreakdown, session_id: Optional[str] = None
        ...
        for rule in sorted_rules:
            ...
            if projected_spend > rule.limit:
                severity = self._determine_severity(
                    projected_spend, rule.limit, rule.alert_thresholds
                )
    ...
    def create_default_rules(
        self, daily_limit: Decimal, monthly_limit: Decimal
    ) -> None:
    ...
    def get_spending_summary(
        self, period: BudgetPeriod, category: Optional[CostCategory] = None
    ) -> Dict[str, Any]:
        ...
        relevant_costs = [
            c for c in self.cost_calculator.cost_history if c.timestamp >= start_date
        ]
                    ...
                    applicable_rules.append(
                        {
                            ...
                            "percentage": (
                                float(total_cost / rule.limit * 100)
                                if rule.limit > 0
                                else 0
                            ),
                        }
    ...
    def get_budget_status(self) -> Dict[str, Any]:
        ...
        recent_violations = [
            ...
            for v in sorted(
                self.violations.values(), key=lambda x: x.timestamp, reverse=True
            )[:10]
        ]
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in cursor.fetchall():
                    rule = BudgetRule(
                        ...
                        categories=(
                            [CostCategory(c) for c in json.loads(row[5])]
                            if row[5]
                            else None
                        ),
                        ...
                        alert_thresholds=(
                            json.loads(row[9]) if row[9] else [0.5, 0.75, 0.9]
                        ),
                        enabled=bool(row[10]),
    ...
    def _get_current_spending(self) -> Dict[str, Decimal]:
        ...
        daily_costs = [
            c for c in self.cost_calculator.cost_history if c.timestamp >= today
        ]
    ...
    def _rule_applies_to_operation(
        self, rule: BudgetRule, cost_breakdown: CostBreakdown
    ) -> bool:
    ...
    def _get_period_spending(
        self, rule: BudgetRule, current_spending: Dict[str, Decimal]
    ) -> Decimal:
    ...
    def _determine_severity(
        self, current_spend: Decimal, limit: Decimal, thresholds: List[float]
    ) -> AlertSeverity:
        ...
        if percentage >= 1.0:
            ...
            max(thresholds[:-1], default=0.75) if len(thresholds) > 1 else 0.75
        ):
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                """,
                    (
                        ...
                        violation.rule_id,
                        violation.rule_name,
                        float(violation.current_spend),
                        float(violation.limit),
                        violation.percentage,
                        violation.period.value,
    ...
    def _trigger_alerts(self, violation: BudgetViolation):
        ...
        # Log the violation
        logger.warning(
            f"Budget violation: {violation.rule_name} - {violation.percentage:.1f}% of limit"
        )
...
def get_budget_enforcer(
    cost_calculator: Optional[CostCalculator] = None, **kwargs
) -> BudgetEnforcer:
