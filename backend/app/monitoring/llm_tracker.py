...
class TraceMetadata(BaseModel):
    ...
    user_id: Optional[UUID] = Field(
        default=None, description="User ID for operation tracking"
    )

    document_id: Optional[UUID] = Field(
        default=None, description="Document ID if processing a document"
    )
    ...
    session_id: Optional[str] = Field(
        default=None, description="Session ID for grouping related operations"
    )

    tags: List[str] = Field(
        default_factory=list, description="Tags for categorizing the operation"
    )

    custom_metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Custom metadata for the operation"
    )
...
class OperationResult(BaseModel):
    ...
    output: Optional[str] = Field(
        default=None, description="Operation output (truncated for large outputs)"
    )
    ...
    cost: Optional[float] = Field(default=None, description="Operation cost in USD")
    ...
    quality_score: Optional[float] = Field(
        default=None, description="Quality assessment of the operation result"
    )
...
class LLMTracker:
        ...
        try:
            trace = client.trace(
                ...
                metadata={
                    ...
                    "document_id": (
                        str(metadata.document_id) if metadata.document_id else None
                    ),
        ...
        try:
            ...
            if generation and is_langfuse_enabled():
                generation.end(
                    ...
                    status_message=(
                        result.error_message if result.error_message else "Success"
                    ),
                )
    ...
    def _store_local_analytics(
        self, metadata: TraceMetadata, result: OperationResult
    ) -> None:
        """
        ...
        try:
            ...
            usage_record = LLMUsage(
                ...
                model_name=metadata.model_name,
                input_tokens=(
                    result.token_usage.input_tokens if result.token_usage else 0
                ),
                output_tokens=(
                    result.token_usage.output_tokens if result.token_usage else 0
                ),
                total_tokens=(
                ...
                processing_duration_ms=(
                    int(result.duration_ms) if result.duration_ms else 0
                ),
                success=result.success,
            ...
            logger.debug(
                f"Local analytics stored for operation: {metadata.operation_type}"
            )
    ...
    def add_user_feedback(
        ...
        try:
            client.score(
                trace_id=trace_id, name="user-feedback", value=score, comment=comment
            )
...
async def _execute_tracked_operation(
    ...
    generation = _tracker.create_generation(
        trace=trace, model_name=model_name, input_text=input_text
    )
        ...
        try:
            if result and hasattr(result, "usage"):
                ...
                token_usage = _tracker.cost_calculator.estimate_tokens(
                    input_text=input_text, output_text=str(result) if result else None
                )

            if token_usage:
                cost = _tracker.cost_calculator.calculate_cost(
                    model_name=model_name, token_usage=token_usage
                )
...
def add_user_feedback(
    trace_id: str, score: float, comment: Optional[str] = None
) -> bool:
