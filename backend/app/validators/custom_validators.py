...
class GoogleDriveIdValidator:
    ...
    def validate(cls, value: str) -> str:
        ...
        if invalid_chars:
            raise ValueError(
                f"Google Drive ID contains invalid characters: {', '.join(invalid_chars)}"
            )
...
class CostValidator:
    ...
    @classmethod
    def validate_cost(
        cls, value: Union[float, Decimal, str], field_name: str = "cost"
    ) -> Decimal:
        """
        Validate cost value.
        ...
        if cost > cls.MAX_SINGLE_OPERATION_COST:
            raise ValueError(
                f"{field_name} exceeds maximum allowed value of ${cls.MAX_SINGLE_OPERATION_COST}"
            )
    ...
    def validate_budget(cls, value: Union[float, Decimal, str]) -> Decimal:
        ...
        if budget > cls.MAX_MONTHLY_BUDGET:
            raise ValueError(
                f"Budget exceeds maximum allowed value of ${cls.MAX_MONTHLY_BUDGET}"
            )
...
class EmbeddingValidator:
    ...
    def validate_embedding(
        cls, value: List[float], expected_dim: int = OPENAI_SMALL_DIM
    ) -> List[float]:
        """
        ...
        if len(value) != expected_dim:
            raise ValueError(
                f"Embedding must have exactly {expected_dim} dimensions, got {len(value)}"
            )
        ...
        for i, val in enumerate(value):
            if not isinstance(val, (int, float)):
                raise ValueError(
                    f"Embedding value at index {i} must be a number, got {type(val).__name__}"
                )

            # Check for invalid values
            if not (-10.0 <= val <= 10.0):  # Very generous range
                raise ValueError(
                    f"Embedding value at index {i} is out of reasonable range: {val}"
                )
...
class FileValidator:
    ...
    def validate_file_size(cls, size_bytes: int) -> int:
        ...
        if size_bytes > cls.MAX_FILE_SIZE:
            ...
            raise ValueError(
                f"File size of {size_mb:.1f}MB exceeds maximum of {max_mb}MB"
            )
...
class TextQualityValidator:
    ...
    def validate_quality_score(
        cls, score: float, field_name: str = "quality score"
    ) -> float:
        """
    ...
    def assess_text_quality(cls, text: str) -> Dict[str, float]:
        ...
        if length < 50:
            ...
            length_score = max(
                0.5, 1.0 - (length - 5000) / 50000
            )  # Penalize very long text

        # Character diversity
        unique_chars = set(text.lower())
        char_diversity = len(unique_chars) / max(
            100, len(text)
...
class ConfigurationValidator:
    ...
    def validate_batch_size(cls, size: int, operation: str = "processing") -> int:
        ...
        max_sizes = {"processing": 50, "embedding": 100, "sync": 1000, "cleanup": 500}
...
class BusinessRuleValidator:
    ...
    @classmethod
    def validate_user_budget_consistency(
        ...
        if monthly_limit < current_spend:
            errors["budget_consistency"] = (
                "Monthly budget limit cannot be less than current month spend"
            )

        # Check for reasonable budget limits
        if monthly_limit > 10000:
            errors["budget_limit"] = (
                "Monthly budget limit exceeds reasonable maximum of $10,000"
            )

        return errors

    @classmethod
    def validate_document_processing_readiness(
        cls, document_data: Dict[str, Any]
    ) -> Dict[str, str]:
        """
