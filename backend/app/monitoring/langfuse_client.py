...
class LangfuseConfig(BaseModel):
    ...
    environment: str = Field(
        default="production", description="Environment name for trace tagging"
    )

    enabled: bool = Field(
        default=True, description="Whether Langfuse tracking is enabled"
    )

    flush_at: int = Field(
        default=15, description="Number of events to batch before flushing"
    )

    flush_interval: float = Field(
        default=0.5, description="Interval in seconds to flush events"
    )
    ...
    timeout: float = Field(default=10.0, description="Timeout for API calls in seconds")
...
class LangfuseClientManager:
    ...
    @classmethod
    def initialize(
        cls, config: Optional[LangfuseConfig] = None
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            trace = self._client.trace(name="health_check", metadata={"test": True})
...
def validate_langfuse_environment() -> Dict[str, Any]:
    ...
    return {
        ...
        "message": (
            "Langfuse environment is properly configured"
            if is_valid
