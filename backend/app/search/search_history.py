...
class SearchQuery:
    ...
    def add_interaction(self, interaction: SearchInteraction):
        ...
        clicks = sum(
            1 for i in self.interactions if i.interaction_type == InteractionType.CLICK
        )
        self.click_through_rate = (
            clicks / max(1, self.total_results) if self.total_results > 0 else 0.0
        )
...
class SearchHistoryManager:
    ...
    async def _create_tables(self):
        ...
        async with self.connection_pool.acquire() as conn:
            ...
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_search_queries_user_timestamp ON search_queries(user_id, timestamp DESC);"
            )
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_search_queries_session ON search_queries(session_id);"
            )
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_search_interactions_query ON search_interactions(query_id);"
            )
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_search_interactions_document ON search_interactions(document_id);"
            )
        ...
        try:
            ...
            async with self.connection_pool.acquire() as conn:
                ...
                """,
                    ...
                    query.timestamp,
                    (
                        json.dumps(asdict(query.query_analysis))
                        if query.query_analysis
                        else None
                    ),
                    query.inferred_intent.value if query.inferred_intent else None,
                    query.search_strategy_used,
                    query.total_results,
                    ...
                    query.refinement_of,
                    (
                        json.dumps(asdict(query.user_context))
                        if query.user_context
                        else None
                    ),
                    json.dumps(query.device_info),
                    json.dumps(query.location_info),
                )
            ...
            if query.user_id:
                ...
                if len(self._recent_queries_cache[query.user_id]) > 100:
                    self._recent_queries_cache[query.user_id] = (
                        self._recent_queries_cache[query.user_id][-100:]
                    )
    ...
    async def record_interaction(
        self, query_id: str, interaction: SearchInteraction
    ) -> bool:
    ...
    async def get_user_search_history(
        self, user_id: str, limit: int = 50, days_back: int = 30
    ) -> List[SearchQuery]:
                ...
                for row in query_rows:
                    ...
                    interactions = [
                        SearchInteraction(
                            ...
                            metadata=(
                                json.loads(irow["metadata"]) if irow["metadata"] else {}
                            ),
                        )
                    ...
                    query = SearchQuery(
                        ...
                        inferred_intent=(
                            SearchIntent(row["inferred_intent"])
                            if row["inferred_intent"]
                            else None
                        ),
                        ...
                        refinement_of=(
                            str(row["refinement_of"]) if row["refinement_of"] else None
                        ),
                        device_info=(
                            json.loads(row["device_info"]) if row["device_info"] else {}
                        ),
                        location_info=(
                            json.loads(row["location_info"])
    ...
    async def analyze_search_patterns(
        self, user_id: str, days_back: int = 90
    ) -> Dict[str, Any]:
        """Analyze user's search patterns and behavior"""
        queries = await self.get_user_search_history(
            user_id, limit=1000, days_back=days_back
        )
        ...
        for query in queries:
            ...
            click_count = len(
                [
                    i
                    for i in query.interactions
                    if i.interaction_type == InteractionType.CLICK
                ]
            )
        ...
        avg_clicks_per_query = (
            sum(click_counts) / len(click_counts) if click_counts else 0
        )
    ...
    async def get_query_suggestions(
        self, partial_query: str, user_id: Optional[str] = None, limit: int = 5
    ) -> List[str]:
