...
async def get_processing_status(
    ...
    try:
        ...
        total_docs_query = select(Document).where(Document.user_id == current_user["id"])
...
async def get_processing_queue(
    ...
    try:
        ...
        queue_query = select(Document).where(
            and_(
                Document.user_id == current_user["id"],
        ...
        return ProcessingQueueResponse(
            ...
            processing_items=[item for item in queue_items if item.processing_status == "processing"],
            queued_items=[item for item in queue_items if item.processing_status == "queued"],
