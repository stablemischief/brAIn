    ...
    try:
        ...
        while current_date <= end_date:
            daily_costs.append({
                ...
                "cost": round(5.50 + (hash(current_date.date().isoformat()) % 1000) / 1000 * 2.0, 2),
        ...
        return CostAnalyticsResponse(
            ...
            cost_per_document=total_cost / 100 if total_cost > 0 else 0,  # Mock document count
            projected_monthly_cost=total_cost * 4.33 if timeframe == "week" else total_cost,
            timeframe=timeframe,
    ...
    try:
        ...
        return UsageAnalyticsResponse(
            ...
            search_patterns={
                ...
                "most_common_terms": ["analysis", "report", "summary", "data", "results"],
...
async def get_budget_status(
    ...
    try:
        ...
        return BudgetResponse(
            ...
            days_remaining_in_month=(datetime.now(timezone.utc) + timedelta(days=32)).replace(day=1) - timedelta(days=1) - datetime.now(timezone.utc),
...
async def get_cost_projections(
    ...
    try:
        ...
        for i in range(projection_days):
            ...
            daily_cost = current_daily_avg * (0.9 + (hash(date.date().isoformat()) % 100) / 500)
            projections.append({
                "date": date.date().isoformat(),
                "projected_cost": round(daily_cost, 2),
                "confidence": 0.85 - (i / projection_days) * 0.2  # Confidence decreases over time
            })
