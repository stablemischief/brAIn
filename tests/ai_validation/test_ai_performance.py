...
class TestAIPerformanceBenchmarks:
    ...
    def performance_test_data(self):
        ...
        return {
            ...
            "medium_documents": [f"Document {i} with more content. " * 20 for i in range(100)],
            "large_documents": [f"Document {i} with extensive content. " * 100 for i in range(500)]
        }
    ...
    def test_text_processing_latency(self, text_processor, performance_test_data):
        ...
        for text_key, max_latency in test_cases:
            ...
            with patch.object(text_processor, 'generate_embedding', return_value=[0.1] * 1536):
    ...
    def test_concurrent_processing_performance(self, text_processor, performance_test_data):
        ...
        for concurrent, max_increase in zip(num_concurrent, max_latency_increase):
            ...
            with ThreadPoolExecutor(max_workers=concurrent) as executor:
                ...
                for i in range(concurrent):
                    ...
                    future = executor.submit(self._process_documents_mock, text_processor, doc_subset)
            ...
            if baseline_time is None:
                ...
                assert latency_increase <= max_increase, \
    ...
    def test_knowledge_graph_building_performance(self, kg_builder, performance_test_data):
        ...
        for set_name, documents, max_time in document_sets:
            ...
            with patch.object(kg_builder, 'extract_entities') as mock_extract:
                mock_extract.return_value = [{"text": "Entity", "type": "PERSON", "confidence": 0.9}]
                with patch.object(kg_builder, 'extract_relationships') as mock_rel:
                    mock_rel.return_value = [{"source": "A", "target": "B", "relation": "KNOWS"}]
                    graph = kg_builder.build_graph(documents)

            end_time = time.time()
            build_time = end_time - start_time

            assert build_time < max_time, \
    ...
    def test_search_performance_scaling(self, search_engine, performance_test_data):
        ...
        for doc_count, max_time in zip(document_counts, max_search_times):
            ...
            documents = [
                {"id": f"doc_{i}", "content": f"Content about machine learning and AI algorithms {i}"}
                for i in range(doc_count)
            ]

            start_time = time.time()
            with patch.object(search_engine, 'calculate_vector_similarity', return_value=0.8):
                results = search_engine.search(query, documents, limit=10)
            end_time = time.time()

            search_time = end_time - start_time

            assert search_time < max_time, \
    ...
    async def test_async_processing_performance(self, text_processor):
        ...
        num_tasks = 50
        texts = [f"Test document {i} with content for processing." for i in range(num_tasks)]
    ...
    def test_cache_performance_impact(self, search_engine):
        ...
        assert speedup >= 1.5, f"Cache speedup {speedup:.2f}x below expected 1.5x minimum"
