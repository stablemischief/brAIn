...
class ContextAwareRanker:
    ...
    async def _calculate_ranking_score(
        ...
        if RankingFactor.RECENCY in ranking_factors:
            score_components[RankingFactor.RECENCY] = self._calculate_recency_score(
                doc_metrics
            )

        # User preference factor
        if RankingFactor.USER_PREFERENCE in ranking_factors and user_context:
            score_components[RankingFactor.USER_PREFERENCE] = (
                await self._calculate_user_preference_score(result, query, user_context)
            )

        # Click-through rate factor
        if RankingFactor.CLICK_THROUGH_RATE in ranking_factors:
            score_components[RankingFactor.CLICK_THROUGH_RATE] = (
                self._calculate_ctr_score(doc_metrics)
            )

        # Document authority factor
        if RankingFactor.DOCUMENT_AUTHORITY in ranking_factors:
            score_components[RankingFactor.DOCUMENT_AUTHORITY] = (
                doc_metrics.authority_score
            )

        # Content quality factor
        if RankingFactor.CONTENT_QUALITY in ranking_factors:
            score_components[RankingFactor.CONTENT_QUALITY] = (
                self._calculate_quality_score(doc_metrics)
            )

        # User context factor
        if RankingFactor.USER_CONTEXT in ranking_factors and user_context:
            score_components[RankingFactor.USER_CONTEXT] = (
                await self._calculate_context_score(
        ...
        if RankingFactor.SEMANTIC_SIMILARITY in ranking_factors:
            score_components[RankingFactor.SEMANTIC_SIMILARITY] = (
    ...
    async def _calculate_user_preference_score(
        self, result: SearchResult, query: str, user_context: UserContext
    ) -> float:
        """Calculate score based on user preferences and history"""
        if (
            not user_context
            or len(user_context.search_history)
            < self.config.min_history_for_personalization
        ):
        ...
        if doc_topics:
            topic_matches = sum(
                user_context.user_preferences.get(topic, 0.0) for topic in doc_topics
            ) / len(doc_topics)
        ...
        if doc_level == user_context.expertise_level:
            ...
            abs(
                self._get_level_numeric(doc_level)
                - self._get_level_numeric(user_context.expertise_level)
            )
            <= 1
        ):
        ...
        if (
            user_context.current_topic_focus
            and user_context.current_topic_focus in doc_topics
        ):
            preference_score += 0.1
    ...
    def _calculate_ctr_score(self, doc_metrics: DocumentMetrics) -> float:
        ...
        if n < 10:  # Small sample size
            ...
            adjusted_ctr = (base_ctr + confidence_adjustment) / (
    ...
    async def _calculate_context_score(
        ...
        if query_analysis.intent_type == "informational" and doc_type in [
            "article",
            "guide",
        ]:
            context_score += 0.15
        elif (
            query_analysis.intent_type == "navigational" and doc_type == "landing_page"
        ):
            context_score += 0.15
        elif query_analysis.intent_type == "transactional" and doc_type in [
            "product",
            "service",
        ]:
            context_score += 0.15
    ...
    async def _combine_scores(
        ...
        if (
            user_context
            and len(user_context.search_history)
            >= self.config.min_history_for_personalization
        ):
            personalization_boost = score_components.get(
                RankingFactor.USER_PREFERENCE, 0.5
            )
            personalized_score = (
    ...
    async def update_document_metrics(
        ...
        if click_occurred:
            ...
            metrics.click_through_rate = metrics.total_clicks / max(
                1, metrics.total_impressions
            )
    ...
    async def update_user_context(
        ...
        if (
            clicked_document_id
            and clicked_document_id not in user_context.clicked_documents
        ):
        ...
        if topic_preferences:
            for topic, weight in topic_preferences.items():
                ...
                user_context.user_preferences[topic] = (
                    0.1 * weight + 0.9 * current_weight
                )

    def get_ranking_explanation(
        self, result: SearchResult, score_components: Dict[RankingFactor, float]
    ) -> Dict[str, Any]:
        """Generate explanation for ranking decision"""
        return {
            "final_score": result.combined_score,
            "components": {
                factor.value: score for factor, score in score_components.items()
            },
            "primary_factors": [
                factor.value
                for factor, score in score_components.items()
    ...
    def _generate_ranking_rationale(
        self, score_components: Dict[RankingFactor, float]
    ) -> str:
        ...
        sorted_factors = sorted(
            score_components.items(), key=lambda x: x[1], reverse=True
        )[:3]
        ...
        for factor, score in sorted_factors:
            if score > 0.7:
                if factor == RankingFactor.RELEVANCE_SCORE:
                    ...
                    explanations.append(
                        "high-quality content from authoritative source"
                    )
