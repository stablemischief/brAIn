...
class CostCalculator:
    ...
    def calculate_cost(
        ...
        if not config.supports_embeddings:
            output_cost = (token_usage.output_tokens / 1000.0) * config.output_price_per_1k
    ...
    def calculate_batch_cost(
        self,
        operations: list[Tuple[str, TokenUsage]]
    ) -> Dict[str, Any]:
        """
    ...
    def compare_model_costs(
        ...
        sorted_comparison = dict(
            sorted(comparison.items(), key=lambda x: x[1]["cost"])
        )
        
        return {
            "token_usage": token_usage.model_dump(),
            "models": sorted_comparison,
            "cheapest": min(comparison.items(), key=lambda x: x[1]["cost"])[0] if comparison else None,
            "most_expensive": max(comparison.items(), key=lambda x: x[1]["cost"])[0] if comparison else None
        }
