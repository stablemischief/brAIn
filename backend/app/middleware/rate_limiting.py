...
class RateLimitStore:
    ...
    def cleanup_old_data(self, max_age_seconds: int = 3600):
        ...
        expired_blocks = [
            client_id for client_id, expiry in self.blocked.items()
...
class RateLimitingMiddleware(BaseHTTPMiddleware):
    ...
    def _check_rate_limit(
        ...
        burst_count = self.store.get_request_count(
            client_id, 
            rule.burst_window_seconds
        )
        
        if burst_count >= rule.burst_requests:
            logger.warning(f"Burst rate limit exceeded for {client_id}: {burst_count}/{rule.burst_requests}")
        ...
        if minute_count >= rule.requests_per_minute:
            logger.warning(f"Minute rate limit exceeded for {client_id}: {minute_count}/{rule.requests_per_minute}")
            return False, {
        ...
        if hour_count >= rule.requests_per_hour:
            logger.warning(f"Hour rate limit exceeded for {client_id}: {hour_count}/{rule.requests_per_hour}")
            return False, {
        ...
        headers = {
            "X-RateLimit-Minute-Limit": str(rule.requests_per_minute),
            "X-RateLimit-Minute-Remaining": str(rule.requests_per_minute - minute_count),
            "X-RateLimit-Hour-Limit": str(rule.requests_per_hour),
