...
class SubscriptionHandler:
    ...
    def __init__(
        self, channel_type: ChannelType, websocket_manager: WebSocketConnectionManager
    ):
    ...
    async def setup_supabase_subscriptions(self) -> bool:
        ...
        raise NotImplementedError(
            "Subclasses must implement setup_supabase_subscriptions"
        )
    ...
    async def broadcast_to_subscribers(
        ...
        if user_filter:
            ...
            for (
                user_id,
                connection_ids,
            ) in self.websocket_manager.user_connections.items():
...
class ProcessingStatusHandler(SubscriptionHandler):
        ...
        try:
            ...
            if user_id:
                await self.websocket_manager.broadcast_to_user(
                    UUID(user_id), MessageType.PROCESSING_STATUS, message_data
                )
    ...
    async def notify_processing_started(
        self, user_id: UUID, document_id: str, task_type: str
    ) -> None:
    ...
    async def notify_processing_completed(
        self, user_id: UUID, document_id: str, result: Dict[str, Any]
    ) -> None:
...
class CostMonitoringHandler(SubscriptionHandler):
    ...
    async def _handle_llm_usage_insert(self, payload: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if user_id:
                await self.websocket_manager.broadcast_to_user(
                    UUID(user_id), MessageType.COST_UPDATE, message_data
                )
    ...
    async def _handle_daily_cost_update(self, payload: Dict[str, Any]) -> None:
        ...
        try:
            ...
            summary_data = {
                ...
                "is_over_budget": record.get("total_cost", 0)
    ...
    async def notify_budget_threshold(
        ...
        message_data = {
            ...
            "percentage_used": (
                current_spending / budget_limit if budget_limit > 0 else 0
            ),
            "timestamp": datetime.utcnow().isoformat(),
        }
...
class SystemHealthHandler(SubscriptionHandler):
    ...
    async def _handle_health_update(self, payload: Dict[str, Any]) -> None:
        ...
        try:
            ...
            await self.broadcast_to_subscribers(MessageType.SYSTEM_HEALTH, message_data)
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            for channel_type, handler in self.handlers.items():
                setup_results[channel_type.value] = (
                    await handler.setup_supabase_subscriptions()
                )
            ...
            if all_success:
                ...
                logger.info(
                    "All real-time subscription handlers initialized successfully"
                )
            else:
