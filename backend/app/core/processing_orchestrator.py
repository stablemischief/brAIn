...
# Local imports
from .text_processor import EnhancedTextProcessor, ProcessingConfig, FileProcessingResult
...
class ProcessingBatch(BaseModel):
    ...
    def is_complete(self) -> bool:
        return all(job.status in [ProcessingStatus.COMPLETED, ProcessingStatus.FAILED, ProcessingStatus.SKIPPED] 
...
class ProcessingConfig(BaseModel):
    ...
    supported_extensions: List[str] = Field(default_factory=lambda: [
        '.pdf', '.docx', '.doc', '.xlsx', '.xls', '.pptx', '.txt', '.html', '.csv', '.md'
    ])
...
class EnhancedProcessingOrchestrator:
    ...
    def create_processing_job(self, file_path: str, priority: ProcessingPriority = ProcessingPriority.NORMAL) -> ProcessingJob:
        """
    ...
    def create_batch_from_directory(self, directory_path: str, 
                                  recursive: bool = True) -> ProcessingBatch:
        """
        ...
        # Create batch
        batch_id = hashlib.sha256(f"{directory_path}_{time.time()}".encode()).hexdigest()[:16]
    ...
    async def process_single_job(self, job: ProcessingJob) -> ProcessingJob:
        ...
        try:
            ...
            if self.config.enable_quality_assessment:
                quality_score = self.quality_assessor.assess_processing_result(processing_result)
            ...
            if success:
                ...
                self.processing_stats.total_chunks_created += len(processing_result.text_chunks)
                self.processing_stats.total_processing_cost += processing_result.cost_estimate
            ...
            if recovery_action == ErrorRecoveryAction.RETRY:
                ...
                print(f"Job {job.job_id} will be retried ({job.retry_count}/{self.config.max_retries})")
    ...
    async def process_batch(self, batch: ProcessingBatch) -> ProcessingSummary:
        ...
        try:
            ...
            success_count = sum(1 for job in batch.jobs if job.status == ProcessingStatus.COMPLETED)
            failure_count = sum(1 for job in batch.jobs if job.status == ProcessingStatus.FAILED)
            skipped_count = sum(1 for job in batch.jobs if job.status == ProcessingStatus.SKIPPED)
            ...
            summary = ProcessingSummary(
                ...
                errors_encountered=[job.error_message for job in batch.jobs if job.error_message],
                performance_metrics={
                    "jobs_per_second": len(batch.jobs) / processing_time if processing_time > 0 else 0,
                    "average_job_time": processing_time / len(batch.jobs) if batch.jobs else 0,
    ...
    def get_batch_status(self, batch_id: str) -> Optional[Dict[str, Any]]:
        ...
        completed_jobs = sum(1 for job in batch.jobs 
                           if job.status in [ProcessingStatus.COMPLETED, 
...
async def process_directory(directory_path: str, recursive: bool = True) -> ProcessingSummary:
