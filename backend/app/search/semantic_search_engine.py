...
from .search_analytics import SearchAnalyticsEngine, AnalyticsReport, MetricType
...
class SemanticSearchEngine:
    ...
    async def search(self, request: SearchRequest) -> SearchResponse:
        ...
        try:
            ...
            if (
                ...
                and self.suggestion_engine
                and len(response.results) >= self.config.suggestion_trigger_threshold
            ):
    ...
    async def _analyze_query(self, request: SearchRequest) -> QueryAnalysis:
        ...
        # Basic analysis
        analysis = await processor.analyze_query(request.query, request.user_context)
        ...
        if self.history_manager and request.user_context:
            similar_queries = await self.history_manager.get_similar_queries(
                request.query, request.user_context.user_id, limit=5
            )
    ...
    async def _determine_strategy(
        self, request: SearchRequest, query_analysis: QueryAnalysis
    ) -> SearchStrategy:
    ...
    def _apply_filters(self, result: SearchResult, filters: Dict[str, Any]) -> bool:
        ...
        if "date_range" in filters:
            ...
            if doc_date:
                if doc_date < date_range.get("start") or doc_date > date_range.get(
                    "end"
                ):
                    return False
    ...
    async def _generate_search_insights(
        ...
        insights = {
            "query_complexity": len(query_analysis.keywords)
            + len(query_analysis.entities),
            "result_diversity": len(
                set(
                    r.metadata.get("document_type", "unknown") for r in response.results
                )
            ),
            "confidence_distribution": {
                "high": len([r for r in response.results if r.combined_score > 0.7]),
                "medium": len(
                    [r for r in response.results if 0.4 <= r.combined_score <= 0.7]
                ),
        ...
        if request.user_context and self.config.enable_personalization:
            ...
            insights["search_history_influence"] = (
                len(request.user_context.search_history) > 5
            )

        return insights

    async def _record_search_event(
        self, request: SearchRequest, response: SearchResponse, strategy: SearchStrategy
    ):
    ...
    def _generate_cache_key(self, request: SearchRequest) -> str:
        ...
        if self.config.enable_personalization and request.user_context:
            user_key = (
                f"{request.user_context.user_id}_{request.user_context.expertise_level}"
            )
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        stats["cache_hit_rate"] = stats["cache_hits"] / max(1, stats["total_queries"])
