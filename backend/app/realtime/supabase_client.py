...
class SupabaseRealtimeConfig(BaseModel):
    ...
    realtime_url: Optional[str] = Field(
        default=None,
        description="Custom realtime URL (auto-generated if None)"
    )
    
    enabled: bool = Field(
        default=True,
        description="Whether real-time functionality is enabled"
    )
    ...
    log_level: str = Field(
        default="info",
        description="Log level for real-time client"
    )
    ...
    def realtime_ws_url(self) -> str:
        ...
        if self.realtime_url:
            return self.realtime_url.replace('http://', 'ws://').replace('https://', 'wss://')
            ...
            try:
                ...
                def on_subscribe(status, error):
                    if error:
                        ...
                        logger.info(f"Channel {channel_name} subscription status: {status}")
...
class SupabaseRealtimeClient:
    ...
    def initialize(
        cls,
        config: Optional[SupabaseRealtimeConfig] = None
    ...
    def is_enabled(self) -> bool:
        ...
        return (
            ...
            self._channel_manager is not None and 
            (self._config.enabled if self._config else False)
        )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            channel_status = self._channel_manager.get_connection_status() if self._channel_manager else {}
            ...
            return {
                ...
                "config": {
                    ...
                    "realtime_url": self._config.realtime_ws_url if self._config else None
                }
            }
...
def validate_supabase_environment() -> Dict[str, Any]:
    ...
    optional_vars = ["SUPABASE_SERVICE_ROLE_KEY", "SUPABASE_REALTIME_URL", "SUPABASE_REALTIME_ENABLED"]
    ...
    return {
        ...
        "message": "Supabase environment is properly configured" if is_valid 
...
def initialize_supabase_realtime(
    ...
    **kwargs
    ...
    final_config = SupabaseRealtimeConfig(
        ...
        service_role_key=config_dict.get("service_role_key", env_config.service_role_key),
        ...
        heartbeat_interval=config_dict.get("heartbeat_interval", env_config.heartbeat_interval),
