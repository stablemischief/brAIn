...
from .text_processor import (
    FileProcessingResult, TextChunk, ProcessingQuality, 
    is_tabular_file, extract_schema_from_csv, extract_rows_from_csv
)
...
class DocumentMetadata(BaseModel):
    ...
    content_language: Optional[str] = Field(default=None, description="Detected language")
    extracted_entities: List[str] = Field(default_factory=list, description="Extracted entities")
    quality_score: float = Field(ge=0.0, le=1.0, description="Overall quality score")
    schema: Optional[Dict[str, Any]] = Field(default=None, description="Schema for tabular data")
...
class DuplicateDetectionResult(BaseModel):
    ...
    similarity_score: float = Field(ge=0.0, le=1.0, description="Similarity to closest match")
    duplicate_file_id: Optional[str] = Field(default=None, description="ID of duplicate file")
    duplicate_chunk_id: Optional[str] = Field(default=None, description="ID of duplicate chunk")
...
class EnhancedDatabaseHandler:
    ...
    def detect_content_duplicate(self, content: str, embedding: List[float], 
                               file_id: str) -> DuplicateDetectionResult:
        """
        ...
        try:
            ...
            hash_response = self.supabase.table("documents").select(
                "metadata->>file_id, metadata->>chunk_index"
            ).eq("metadata->>chunk_hash", content_hash).neq(
            ...
            all_chunks_response = self.supabase.table("documents").select(
                "embedding, metadata->>file_id, metadata->>chunk_index, id"
            ).neq("metadata->>file_id", file_id).limit(1000).execute()
            ...
            if len(words) > 5:  # Only for substantial content
                similar_response = self.supabase.table("documents").select(
                    "content, metadata->>file_id, id"
                ).neq("metadata->>file_id", file_id).limit(100).execute()
                        ...
                        if len(other_words) > 5:
                            ...
                            if jaccard >= 0.8:  # High word overlap
                                return DuplicateDetectionResult(
                                    ...
                                    confidence=jaccard * 0.8,  # Lower confidence for heuristic
    ...
    def check_document_exists(self, file_id: str) -> bool:
        ...
        try:
            response = self.supabase.table("documents").select(
                "id"
            ).eq("metadata->>file_id", file_id).limit(1).execute()
    ...
    def delete_document_by_file_id(self, file_id: str) -> bool:
        ...
        try:
            # Delete documents
            doc_response = self.supabase.table("documents").delete().eq(
                "metadata->>file_id", file_id
            ).execute()
            
            # Delete document rows (for tabular data)
            rows_response = self.supabase.table("document_rows").delete().eq(
                "dataset_id", file_id
            ).execute()
            
            # Delete metadata
            meta_response = self.supabase.table("document_metadata").delete().eq(
                "id", file_id
            ).execute()
            
            print(f"Deleted document {file_id}: {len(doc_response.data)} chunks, "
    ...
    def insert_document_metadata(self, metadata: DocumentMetadata) -> bool:
        ...
        try:
            ...
            # Upsert the record
            response = self.supabase.table("document_metadata").upsert(db_data).execute()
            ...
            for chunk in chunks:
                ...
                if check_duplicates:
                    ...
                    if duplicate_result.is_duplicate:
                        duplicate_count += 1
                        print(f"Skipping duplicate chunk: {duplicate_result.detection_method} "
    ...
    def process_file_for_rag(self, processing_result: FileProcessingResult, 
        ...
        try:
            ...
            for chunk in processing_result.text_chunks:
                ...
                db_chunk = DocumentChunk(
                    ...
                    content=chunk.content,
                    chunk_index=chunk.chunk_index,
                    embedding=[0.0] * 1536,  # Placeholder - should come from actual embeddings
                    quality_score=chunk.quality_score,
            ...
            print(f"Processed {processing_result.file_name}: "
    ...
    def get_processing_statistics(self, file_id: Optional[str] = None) -> ProcessingStats:
        """
        ...
        try:
            ...
            total_cost = sum(Decimal(str(doc.get("processing_cost", 0))) 
                           for doc in meta_response.data)
            avg_quality = sum(doc.get("quality_score", 0.0) 
                            for doc in meta_response.data) / total_files
