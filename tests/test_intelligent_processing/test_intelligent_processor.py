...
def mock_ai_client():
    ...
    mock_message.content[0].text = '{"document_type": "text", "category": "general", "complexity": "moderate", "confidence": 80}'
...
class TestIntelligentFileProcessor:
    ...
    async def test_process_json_file(self, temp_json_file, sample_processing_context):
        ...
        assert result.processing_quality > 0.0
        assert "json" in result.extraction_method.lower() or "structured" in result.extraction_method.lower()
    
    @pytest.mark.asyncio
    async def test_batch_processing(self, temp_text_file, temp_json_file, sample_processing_context):
...
class TestFormatOptimizationEngine:
    ...
    async def test_strategy_selection(self, temp_text_file):
        ...
        assert strategy.optimization_level in [OptimizationLevel.BALANCED, OptimizationLevel.SPEED]
    ...
    def test_optimization_strategies(self):
        ...
        pdf_strategies = [s for s in engine.strategies.values() if "pdf" in s.supported_formats]
        text_strategies = [s for s in engine.strategies.values() if "txt" in s.supported_formats]
        json_strategies = [s for s in engine.strategies.values() if "json" in s.supported_formats]
...
class TestIntelligentErrorHandler:
    ...
    async def test_recovery_strategy_selection(self, sample_processing_context):
        ...
        recovery_plan = await handler._generate_recovery_plan(analysis, sample_processing_context)
    ...
    def test_error_pattern_matching(self):
        ...
        category, severity = handler._classify_error_pattern(error, str(error), "")[0], handler._classify_error_pattern(error, str(error), "")[1]
...
class TestConvenienceFunctions:
    ...
    @pytest.mark.asyncio
    async def test_process_file_with_ai(self, temp_text_file, sample_processing_context):
    ...
    @pytest.mark.asyncio
    async def test_batch_process_with_ai(self, temp_text_file, temp_json_file, sample_processing_context):
...
class TestIntegration:
    ...
    @pytest.mark.asyncio
    async def test_quality_feedback_loop(self, temp_text_file, sample_processing_context):
        ...
        assert result_high.extraction_method != result_speed.extraction_method or \
               result_high.optimization_applied != result_speed.optimization_applied
...
class TestPerformance:
    ...
    @pytest.mark.asyncio
    async def test_processing_performance(self, temp_text_file, sample_processing_context):
