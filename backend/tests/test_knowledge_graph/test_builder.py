...
class TestKnowledgeGraphBuilder:
    ...
    @pytest.mark.asyncio
    async def test_process_document_success(self, test_config, sample_entities, sample_relationships):
        ...
        builder.relationship_detector.detect_relationships.return_value = sample_relationships
        
        builder.storage = AsyncMock()
        builder.storage.create_node.side_effect = [
            GraphNode(id=uuid4(), name="AI", entity_type=EntityType.CONCEPT, confidence=0.9),
            GraphNode(id=uuid4(), name="ML", entity_type=EntityType.CONCEPT, confidence=0.85)
        ]
        ...
        result = await builder.process_document(
            text="AI and Machine Learning are related",
    ...
    async def test_process_document_no_entities(self, test_config):
        ...
        document_id = uuid4()
        result = await builder.process_document(
            text="This text has no extractable entities",
            document_id=document_id
        )
