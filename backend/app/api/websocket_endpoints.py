...
class WebSocketConnectionRequest(BaseModel):
    ...
    session_id: Optional[str] = Field(
        None, description="Session ID for grouping connections"
    )
    channels: List[str] = Field(
        default_factory=list, description="Initial channels to subscribe to"
    )
...
class BroadcastRequest(BaseModel):
    ...
    scope: MessageScope = Field(
        default=MessageScope.BROADCAST, description="Message scope"
    )
    ...
    try:
        ...
        if connection_id:
            await websocket_manager.disconnect(
                connection_id, reason="Connection closed"
            )
...
async def broadcast_message(
    ...
    try:
        if not broadcaster:
            raise HTTPException(
                status_code=503, detail="Message broadcaster not available"
            )
...
async def get_channels(
    ...
    try:
        ...
        return {"total_channels": len(channels), "channels": channels, "stats": stats}
...
async def get_channel_info(
    channel: str, manager: WebSocketConnectionManager = Depends(get_websocket_manager)
) -> Dict[str, Any]:
